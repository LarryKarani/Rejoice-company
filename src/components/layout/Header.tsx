    ...
    >
      <div className="container flex items-center justify-between">
        <Link to="/" className="flex items-center space-x-2">
          <Users size={32} className={`transition-colors duration-300 ${isScrolled ? 'text-primary' : 'text-white'}`} />
          <span className={`font-semibold text-xl transition-colors duration-300 ${isScrolled ? 'text-gray-800' : 'text-white'}`}>
            Rejoice Recruitment
          </span>
        </Link>
        ...
        <nav className="hidden lg:flex items-center space-x-8">
          ...
          <Link 
            to="/contact" 
            className={`btn ${isScrolled ? 'btn-primary' : 'bg-white text-primary hover:bg-gray-100'}`}
          >
            Get Started
          </Link>
        </nav>
